...
class CubicBezier(object):
    ...
    def length(self, approx=True):
        """Calculate the length of the path up to a certain position"""

        cdef int subdivisions = 1000
        cdef double length = 0
        cdef double delta = 1/subdivisions
        cdef double distance = 0

        if approx:
                chordLen = abs(self.end-self.start)
                controlnetLen = abs(self.control1-self.start) + abs(self.control2-self.control1) + abs(self.end-self.control2)
                length = (chordLen+controlnetLen)/2.       	
